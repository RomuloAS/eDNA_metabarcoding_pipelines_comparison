...
# Global parameters
NCPUS=10                    # Number of CPUs to use. A lot of the steps are parallelized (int, 1+)
...
CROP_LENGTH=126             # Cut reads to this length after filtering. Just under amplicon length

# Merging reads with flash
MIN_OVERLAP=15              # Minimum number of overlapping nucleotides to merge reads (int, 1+)
MAX_OVERLAP=126             # Maximum number of overlapping nucleotides to merge reads (int, 1+)

# Extracting barcodes
MAX_PRIMER_DIFF=2           # Maximum number of differences allowed between primer and sequence (int, 0+)
...
QUERY_COV=0.85              # At least that proportion of the sequence must match the database (float, 0-1)

# Filters
MIN_HIT_LENGTH=90           # Minimum vsearch hit length to keep in results (int, 1+)
